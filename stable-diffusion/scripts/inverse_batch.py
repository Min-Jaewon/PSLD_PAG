...
import datetime
import pdb
from ldm.data.dataloader import FFHQDataset, get_dataloader
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="start at the min batch",
    )
    parser.add_argument(
        "--max_count",
        type=int,
        default=-1,
        help="stop at the max batch",
    )
    parser.add_argument(
        "--inpainting_seed",
        type=int,
        default=-1,
        help="the seed for inpainting task (for reproducible sampling)",
    )
    parser.add_argument(
        "--precision",
        type=str,
    ...
    #     assert opt.adg_scale!=0.0, 'adg_scale must be larger than 0.0'
    # else:
    #     assert opt.adg_scale==0.0, 'adg_scale must be 0.0'
    ...
    dataset= FFHQDataset(opt.data_path, transforms=transform)
    loader= get_dataloader(dataset, batch_size=opt.n_samples, num_workers=0, train=False)
    
    
    for i, batch in enumerate(loader):
        if i>=opt.min_count:
            if i==opt.max_count: break
            ...
            org_image = torch.clone(x_checked_image_torch.detach())
            ...
            if measure_config['operator'] ['name'] == 'inpainting':
                ...
                mask_list=torch.split(mask, 1, dim=0)
                duplicate_mask=[batch_mask[:,0,:,:].repeat(3,1,1).unsqueeze(0) for batch_mask in mask_list]
                mask = torch.cat(duplicate_mask, dim=0)
            ...
            #########################################################
            def save_np_as_png(imgs, path, ids):
                for i in range(imgs.size(0)):
                    ...
                    id=ids[i].split('.')[-2][-5:]
                    img.save(os.path.join(path, id+'.png'))

            y_n_path=os.path.join(outpath, 'y_n')
            os.makedirs(y_n_path, exist_ok=True)
            save_np_as_png(y_n.clone().detach(), y_n_path, ids)
            ...
            for id in ids:
                id=id.split('.')[-2][-5:]
                with open(os.path.join(outpath, 'logs', f"{id}.txt"), 'w') as file:
                    file.write('File id: '+ str(id)+'\n'
                            + 'Seed: '+ str(torch.initial_seed()) + '\n'
                            + 'Generate Time: ' + str(datetime.datetime.now()) + '\n'
                            + 'Args: ' + str(opt) + '\n'
                            + 'Seed Dict' + str(seed_dict))
                with open(os.path.join(outpath, 'seeds', f'{id}.pt'), 'wb') as seed_log:
                            ...
                            if not opt.skip_save:
                                for i, x_sample in enumerate(x_checked_image_torch):
                                    ...
                                    if opt.inpainting:  # Inpainting gluing logic as in SD inpaint.py
                                        image = torch.clamp((org_image[i,:,:,:]+1.0)/2.0, min=0.0, max=1.0)
                                        image = image.cpu().numpy()       
                                        mask_i = mask[i,:,:,:].cpu().numpy()
                                        inpainted = mask_i*image+(1-mask_i)*x_sample.cpu().numpy()
                                        inpainted = inpainted.transpose(1,2,0)*255
                                        Image.fromarray(inpainted.astype(np.uint8)).save(os.path.join(sample_path, f"{id}.png"))
                                    else:
